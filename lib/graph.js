...
Graph.prototype.evaluate = function(node,level){
  var stack =this.clone(node.postfixStack); //cópia do objecto
  return this.evaluator.evaluate(stack,level);
  ...
  while(node!==undefined){
    ...
    switch(node.type) {
      case Definition.BEGIN:
        break;
      case Definition.PROCESS:
        self.evaluate(node,level);
        self.emit('memoryUpdate', JSON.stringify(self.memory.vars));
        break;
      case Definition.IF:
        level++;
        var boolResult=self.evaluate(node,level);
        if(boolResult===true){
          node=node.nexttrue;
      ...
      case Definition.JOIN:
        ...
        break;
      case Definition.WRITE:
        self.console=self.evaluate(node,level);
