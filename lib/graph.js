...
var Graph = function() {
    ...
    this.step = function() {
      if(self.stepping){
        if(self.previousNode===undefined){
          self.emit('highlight',{'actual':self.stepNode.uuid, 'previous':undefined});
        }
        else{
          self.emit('highlight',{'actual':self.stepNode.uuid, 'previous':self.previousNode.uuid});
        }
      }
      if(self.stepNode!==undefined){
        ...
        switch(self.stepNode.type) {
          ...
          case Definition.IF:
            ...
            if(boolResult===true){
              self.previousNode=this.stepNode;
        ...
        if(!self.previousWasIf && self.stepNode.type!=Definition.READ) {
            ...
            /*if(self.stepNode===undefined){
            //o fluxograma acabou quando o stepNode é undefined
            if(self.stepping){
              self.memory.vars=[];
            }
            console.log(self.memory.vars);
            self.emit('memoryUpdate',JSON.stringify(self.memory.vars));
            self.emit('finished');
            console.log("TERMINOU O FLUXOGRAMA");
            return true; //terminou
        ...
        if(self.stepping){
          self.emit('stepProcessed'); //informa o IDE que o nó já foi processado
        }
        else{
          self.step.apply(); //executa o próximo nó automaticamente
        }
      }
      else{
